...
```vim new.xml нажать i написать текст:```

 ```html
 <root>
	<firstName>Anna</firstName>
	<lastName>Mugu</lastName>
  	<age>25</age>
	<numerOfPets>2</numerOfPets>
   	<desiredSalary>250.000 $</desiredSalary>
 ...
 8. Отправить изменения на внешний репозиторий.
  ...
  ```git add new.xml```  ```git commit -m "second_commit```  ```git push```
...
```cd TXT``` далее ввести команду ```touch new.txt```
...
```vim new.txt нажать i написать текст:```
...
```git add new.txt```  ```git commit -m "second_commit```  ```git push```
...
10. В файл preferences.txt” добавить информацию о своих предпочтениях (Любимый фильм, любимый сериал, любимая еда, любимое время года, сторона которую хотели бы посетить) в формате TXT.
 ...
 Ввести команду ```vim preferences.txt``` нажать ```i``` Ввести:

```
favourite Movie: Spider-Man
favourite Series: True Detective
favourite Food: Taco
...
Ввести команду ```vim skills.txt``` нажать ```i``` Ввести:

```
Skills:Testing Theory, Git, Charles, Postman, SQL, Python
```
...
Author: Создатель баг-репорта;
Executor: Разработчик, который будет устранять баг;
Steps To Reproduce: Точная последовательность действий, которая приводит к тому, чтобы воспроизвести баг;
actual Result: К чему приводит воспроизведение шагов, как сейчас работает программа;
expected Result: Как должна работать программа на самом деле и к чему должны приводить действия, описанные в пункте «Шаги к воспроизведению»;
attachment: Ссылки, скриншоты, видео и другие материалы, которые помогут исполнителю лучше понять суть проблемы;
```
